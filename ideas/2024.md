...
The existing support is very brittle and unpleasant to read and only support dependencies fetched via git. 

This project would be interesting as it touches seveal different types of projects:

1. redesigning the build infrastructure to be easier to hack on (nixpkgs)
2. implmenting the new build infrastructure (nixpkgs)
3. extending the `crystal2nix` tool to understand other VCSs (crystal2nix)

Skills required/preferred:
Crystal because crystal2nix is written in it.
