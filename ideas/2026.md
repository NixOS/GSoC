...
|-|-|-|
| [Improve matching quality with more structured data and heuristics](https://github.com/NixOS/nix-security-tracker/issues/391) | medium | |
| [Implement workflow for change proposals by maintainers](https://github.com/NixOS/nix-security-tracker/issues/325) | hard | |
| [Increase data storage efficiency](https://github.com/NixOS/nix-security-tracker/issues?q=is%3Aissue%20state%3Aopen%20label%3Adata%20label%3Aperformance%20-label%3Adeployment) | medium to hard | |
| [Dig into data to increase transparency or observability](https://github.com/NixOS/nix-security-tracker/issues?q=state%3Aopen%20label%3Aobservability) | medium | |
| Implement real-time collaborative editing features (e.g. presence indication, [live updates](https://github.com/NixOS/nix-security-tracker/issues/439)) | medium to hard | |
| Any of the [user stories](https://github.com/NixOS/nix-security-tracker/issues?q=is%3Aissue%20state%3Aopen%20label%3A%22user%20story%22) | medium to hard | a complex one, or a small set of non-trivial ones |
| Fix [UI problems](https://github.com/NixOS/nix-security-tracker/issues?q=is%3Aissue%20state%3Aopen%20label%3Askin) or [code quality issues](https://github.com/search?q=repo%3ANixOS%2Fnix-security-tracker%20FIXME&type=code) | medium | |
| Introduce new tools or paradigms into the development process | medium | e.g. property testing of data ingestion, testing migrations, visual regression tests, rendering documentation from tests, ... |
...
- Valentin Gagarin [@fricklerhandwerk](https://github.com/fricklerhandwerk)
  ...
  Please reach out on Matrix for questions: [@fricklerhandwerk:matrix.org](https://matrix.to/#/@fricklerhandwerk:matrix.org)
...
A good path to start out is engaging with the code and showing your work:

- [Get the system to run locally](https://github.com/NixOS/nix-security-tracker/blob/main/CONTRIBUTING.md#running-the-service-in-a-development-environment) and try it out

  Open pull requests to improve the instructions where they were lacking.

- Read through the issue tracker

  You'll likely find something that stood out as incomplete or inadequate.
  Ask questions or suggest implementation approaches.

- Read tests and skim code; write tests to check your assumptions

  Open pull requests with your new or refined tests to validate what you've learned.
  Make small fixes and refactorings as you go.

An *ideal* proposal consists of:

- Verification procedures for deliverables

  Where possible by nature of the problem, verification should be literally tests, in a pull request merged during the application phase.
  Manual testing should be a justified exception.

- An outline of the implementation approach

  Please discuss drafts early on, to avoid running into non-obvious traps in the current code base.

- Task breakdown with time estimates

  Make a table with at least weekly and at most daily granularity (e.g. "Create and test migrations for X; 2d").
  Don't forget to take into account the review cycle, weekends, planned vacation, and the possibility of fixups required after exposing your work to users.
...
The [build hook](https://nix.dev/manual/nix/2.34/command-ref/conf-file#conf-build-hook) is a program invoked by Nix when performing remote builds. It is responsible for selecting a remote builder machine and ultimately calling the underlying remote store build methods. Removing the build hook has been on the todo list for a long time.

Today, the build hook exists for two main reasons: to deal with the fact that calling a remote store's build method is a blocking call, and to provide a way to implement custom remote building logic without modifying Nix itself. The remote store build method being a blocking call is a problem we intend to solve before work on this project starts by the GSoC participant, and can be considered a prerequisite. Providing a way to implement custom remote building logic is intended to be part of this effort, and can be done by adding some new virtual store/builder methods that have a signature similar to what the build hook protocol consumes today. These methods could be implemented by a plugin or daemon providing a custom store. See the below linked issues for more details.
...
- [Remove the build hook mechanism](https://github.com/NixOS/nix/issues/1221)
- [New virtual store method(s) to support builder stores](https://github.com/NixOS/nix/issues/15533)
- [`nix-build --rebuild` does not work with remote builders](https://github.com/NixOS/nix/issues/10451)
...
## Provide support for Mandatory Access Control

Effort: Large (350 hours)

Currently, NixOS lacks first-class support for Mandatory Access Control (MAC), which is fundamental for maintaining a strong security posture for both individuals and organizations.

The most widely used MAC implementations on Linux distributions are SELinux and AppArmor. Both are challenging to integrate with NixOS due to the immutable Nix store and its non-FHS-compliant filesystem layout. SELinux may be particularly difficult to implement in NixOS compared to AppArmor, as it typically relies on extended filesystem labels and metadata that would need to be applied to (immutable) Nix store paths. However, I'm not a NixOS expert, so I don't really know which MAC implementation fits NixOS best, or whether a completely new NixOS-specific MAC solution would be needed.

Skills required:

- NixOS internals
- Mandatory Access Control
- Linux MAC implementations (e.g., SELinux, AppArmor)

Possible mentors:

- RossComputerGuy

Difficulty: Very Hard (requires significant mentor effort)
...
- https://github.com/NixOS/nixpkgs/pull/359817
- https://github.com/NixOS/nixpkgs/pull/396168
- https://discourse.nixos.org/t/selinux-on-nixos/62729/3
