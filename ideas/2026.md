...
This project is to migrate some standard service modules to modular services. Pick a module from the module list, see if it makes sense to migrate it, and make a PR for it. Can be extended to arbitarily many modules, depending on progress.
(For now this is about service *implementations*. User-facing options should remain unchanged.)
...
## Generative Nix: LLM Best Practices and Tooling
...
- Identify best practices for Nix flake development
- Identify best practices for Nixpkgs module development
- Encode these practices into an `AGENTS.md` tooling directive file
- Demonstrate improved code generation and output for an LLM of their choice when using this file

Deliverables:
- `AGENTS.md` that can help improve LLM performance on Nix-related tasks
- Blog post describing discovered best practices and tooling directive

Skills required:
- Basic experience with LLMs
- Access to commercial LLMs and/or open-source LLMs
...
- Experience writing `AGENTS.md`, `claude.md`, or similar agent tooling directives
...
- https://platform.claude.com/docs/en/agent-sdk/modifying-system-prompts#methods-of-modification (Claude-specific, but ideas transfer)
