...
Nix together with the familiar combination of devhells and direnv have elevated developer convenience tremendously for virtually all projects within the Nix ecosystem and for many outside of it, as well.
So that's great and we want that. But there's a dark downside.
Entering these devshells typically involves arbitrary code execution.
What we need is some development within this tool space that would allow us to maintain the level of convenience we have worked hard to achieve
but without the risky "rolling of the dice" that is currently involved.
It was said that [bubblewrap](https://github.com/containers/bubblewrap) could be of great help here.
