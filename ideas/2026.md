...
Currently, NixOS does not provide first-class, end-to-end support for a complete chain of trust that validates the integrity of the entire machine from firmware to userspace. A typical chain of trust verifies the following components in sequence:
- Firmware / UEFI (root of trust)
- TPM measurements (measured boot)
- First-stage bootloader
- Second-stage bootloader or Unified Kernel Image (UKI)
- OS kernel
- Kernel modules
- Userspace
The most common mechanism used to implement a chain of trust on PCs is UEFI Secure Boot, often combined with measured boot using a TPM.

Within the NixOS ecosystem, most Secure Boot–related efforts have converged around the Lanzaboote project. Lanzaboote allows signing a per-generation Unified Kernel Image (UKI) that bundles the kernel, initrd, and boot logic, enabling verification up to kernel startup. However, this approach does not yet provide a complete, end-to-end chain of trust. In particular, it lacks:

- Userspace verification — integrity verification of the root filesystem and userspace binaries after boot (e.g., `dm-verity`, `fs-verity`, or similar mechanisms).
- PKCS#11 support — integration with external hardware-backed keys (e.g., TPMs or HSMs) for signing and key management.

Skills required:

- Chain of trust and Secure Boot concepts
- NixOS boot stack
...
Prior efforts:
- https://github.com/NixOS/nixpkgs/blob/0ccc0f9894f6b89032df99ff1cbee7586c61024f/nixos/modules/image/repart-verity-store.nix
- https://github.com/numinit/nixpkcs
